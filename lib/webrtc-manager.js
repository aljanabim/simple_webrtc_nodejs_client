...
class WebrtcManager {
    ...
    signalingMessageHandler(message) {
        ...
        switch (action) {
            ...
            case "close":
                ...
                break;
            case "sdp":
                console.log(`Received ${sdp.type} from ${from}`);
            ...
            case "ice":
                ...
                break;
            default:
                console.log(`Received un an unkown action ${action}`);
                break;
        }
    }
    ...
    addPeer(peerId, peerType, polite) {
        if (peerId in this.peers) {
            console.log("A peer connection with", peerId, "already exists");
        } else {
    ...
    async updateSessionDescription(peer, description) {
        try {
            ...
            if (peer.ignoreOffer) {
                console.log("Peer offer was ignore because we are impolite");
                return;
            }
    ...
    removePeer(peer) {
        if (peer) {
            ...
            console.log(`Connection with ${peer.peerId} has been removed`);
        }
    }
}
