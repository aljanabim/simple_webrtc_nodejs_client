...
class SignalingChannel {
     ...
     * @param {string} peerType - What type of peer this is, this is up to the application. For example peerType can be 'admin', 'vehicle' or 'controltower'
     * @param {string} hostURL - URL to the signaling server
     * @param {string} token - The token used to authenticate the connection to the signaling server
     * @memberof SignalingChannel
     */
    constructor(peerId, peerType, hostURL, token) {
        this.peerId = peerId;
        ...
        this.socket = new io(hostURL, {
            ...
            reconnection: true, // disables auto reconnection to server, this can occur when for example the host server disconnects. When set to true, the client would keep trying to reconnect
    ...
    resetListeners() {
        ...
        this.onDisconnect = () => console.log("Disconnected from signaling server");
        this.onError = (error) => console.log("Signaling Server ERROR", error);
        this.onReconnect = (nr) => console.log("Reconnected to signaling server, with", nr, "attempts");
        this.onMessage = (message) => console.log(util.inspect(message, { showHidden: false, depth: null }));
